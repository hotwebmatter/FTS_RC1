...
function fixInputs(type, className){
	...
	jQuery('input[type="' + type + '"]').each(function () {
		if (jQuery(this).parent('label').length > 0){
			...
			var text = '';
			if (jQuery(this).parent().contents().length > 1){
				if (jQuery(this).parent().contents()[nodeIndex+1].nodeType === this.TEXT_NODE){
...
function initRadioButtons(){
	"use strict";
    jQuery('label.art-radiobutton').each(function(){
        if (jQuery('span.typesummary', jQuery(this)).length > 0){
            var summarySpan = jQuery('span.typesummary', jQuery(this)).eq(0).outerHTML();
            jQuery('span.typesummary', jQuery(this)).css('display','none');
            jQuery(this).parent().append(summarySpan);
        }
    });
	var className = 'art-radiobutton';
	jQuery.each(jQuery('label.' + className), function (i, val) {
		var label = jQuery(val),
			ctrl = new Control();
		var artRadioButtonProcess = function() {
...
jQuery(function ($) { // Changes to apply Artisteer styles
    'use strict';
 //   $('input[type="submit"][class!="art-search-button"]').addClass('art-button');
  //  $('button').addClass('art-button');
    $('.blogsite thead').addClass('art-postmetadataheader');
});
